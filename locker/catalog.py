"""Registre partage pour les lanceurs Locker."""
...
THEME_SEQUENCE = ["classical", "symmetric", "asymmetric", "hashing", "signatures", "applications"]
...
def resolve_target(token):
    if isinstance(token, tuple):
        return MODULE_CATALOG.get(token)
    if token in COURSE_ALIASES:
        return MODULE_CATALOG.get(COURSE_ALIASES[token])
    if "." in token:
        theme, slug = token.split(".", 1)
        return MODULE_CATALOG.get((theme, slug))
    return None


def iter_modules_for_theme(theme: str):
    for (theme_name, slug), (module_path, label) in MODULE_CATALOG.items():
        if theme_name == theme:
            yield slug, module_path, label
